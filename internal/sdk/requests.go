...
// GetRequestByID - Returns the details of a request.
...
// RevokeRequestByID - Revokes the specified request.
func (s *requests) RevokeRequestByID(ctx context.Context, request operations.RevokeRequestByIDRequest) (*operations.RevokeRequestByIDResponse, error) {
	baseURL := utils.ReplaceParameters(s.sdkConfiguration.GetServerDetails())
	url, err := utils.GenerateURL(ctx, baseURL, "/requests/{request_id}/revoke", request, nil)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}

	bodyReader, reqContentType, err := utils.SerializeRequestBody(ctx, request, "RequestRevokeParams", "json")
	if err != nil {
		return nil, fmt.Errorf("error serializing request body: %w", err)
	}
	if bodyReader == nil {
	...
	res := &operations.RevokeRequestByIDResponse{
		StatusCode:  httpRes.StatusCode,
		ContentType: contentType,
		RawResponse: httpRes,
	}
